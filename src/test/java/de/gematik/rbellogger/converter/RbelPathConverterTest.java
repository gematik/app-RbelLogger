...
public class RbelPathConverterTest {
    ...
    public void convertAbsolutePathWithQueryParameters() {
        final RbelElement rbelElement = RbelConverter.build().convertMessage("/foobar?"
            + "&nonce=997755"
            + "&client_id=fdsafds"
    ...
    public void domainAndProtocolAndQuery_shouldConvertNested() {
        final RbelElement rbelElement = RbelConverter.build().convertMessage(
        ...
        assertThat(((RbelPathElement) rbelElement).getQueryParameter().getChildElements().get("code"))
            .isInstanceOf(RbelJweElement.class);
    }

    @Test
    public void simpleUrlWithoutQuery() {
        final RbelElement rbelElement = RbelConverter.build().convertMessage("http://redirect.gematik.de/foo/bar");
